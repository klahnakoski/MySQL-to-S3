...
* **`reference_only`** - *`<table>.<column>`* - Some tables are used to lookup primitive values, or maybe you are not interested in the properties for a given table: In these cases you can have the foreign key replaced with the canonical value that foreign key represents. For example: `user_id` refers to the `users` table, which has a `email` column. Everywhere there is a `user_id` column, the foreign key is replaced with the `email` value. This greatly simplifies the JSON at the risk of loosing some information. If just the table is named, then it is included, but no nested documents will be attached.   
